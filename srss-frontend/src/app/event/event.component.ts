...
export class EventComponent implements OnInit, OnDestroy {
  ...
  protected readonly EvaluationMethod = EvaluationMethod;
    ...
    this.route.paramMap.subscribe((pm: ParamMap) => {
      ...
      this.eventService.getEventByUuid(uuid).subscribe((event: Event) => {
        ...
        this.eventService.getAllUsersEventsByEventUuid(uuid).subscribe(ueList => {
          ...
          // После того как получили usersEvent и eventWeights (см. ниже),
          // создаём/обновляем FormControl для MAN_HOURS
        ...
        this.eventService.getCategoryEvaluationMethodByUuid(event.categoryEvaluationMethodUuid)
          .subscribe(cem => {
            ...
            this.eventService.getEventWeightsByCategoryEvaluationMethodUuid(cem.uuid).subscribe((weights: EventWeight[]) => {
              ...
              // Также после загрузки весов пересоздаём контролы, если уже есть usersEvent
              this.initializeScoreControls();
            });
          });
  ...
  // Инициализация FormControl для MAN_HOURS: создаём только если метод MAN_HOURS
  private initializeScoreControls(): void {
    ...
    this.usersEvent.forEach(ue => {
      ...
      const eventWeight = this.eventWeights.find(e => e.uuid === weightUuid) || {weight: 0};
      const initialScore = (ue.userScore != null && eventWeight.weight !== 0) ? ue.userScore / eventWeight.weight : 0;
      const control = new FormControl(initialScore, [
        Validators.required,
      ...
      control.valueChanges
        ...
        .subscribe(val => {
          if (control.valid) {
            this.eventService.setUserScore(ue.uuid, weightUuid, val).subscribe();
          }
        });
    });
  }
